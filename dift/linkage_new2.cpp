...
static inline void sys_getuid32_start (struct thread_data* tdata) {
    ...
    if (tdata->recheck_handle) {
#ifdef FW_SLICE
	printf ("[SLICE] #00000000 #call getuid32_recheck [SLICE_INFO]\n");
#endif
...
TAINTINT fw_slice_stringstring_internal (ADDRINT ip, char* inst_str, ADDRINT src_mem_loc, ADDRINT eflags, uint32_t count_reg, ADDRINT counts, UINT32 op_size, u_long dst_mem_loc, uint32_t first_iter) { 
	...
	if (count_reg) { //count_reg is set only with REP prefix
		assert (count_reg == 9); //should always be ecx, otherwise, add the support
		tainted = fw_slice_memmemreg (ip, inst_str, ea_src_mem_loc, ea_dst_mem_loc, size, size, count_reg, 4, counts, 0);
	} else{ 
...
TAINTINT fw_slice_stringreg_internal (ADDRINT ip, char* inst_str, ADDRINT src_mem_loc, ADDRINT eflags, uint32_t count_reg, ADDRINT counts, UINT32 op_size, 
		uint32_t reg, uint32_t reg_size, uint32_t is_upper8, uint32_t reg_value, uint32_t first_iter) { 
	//only check on the first iteration
    if (first_iter) {
        int size = (int) (counts*op_size);
        int tainted = 0;
	...
	if (count_reg) { //count_reg is set only with REP prefix
		...
		tainted = fw_slice_memreg (ip, inst_str, reg, reg_size, reg_value, is_upper8, ea_src_mem_loc, size);
	}
	//assert for DF_FLAG
	assert (is_flag_tainted(DF_FLAG) == 0);
        return tainted;
...
static inline void fw_slice_src_stringreg (INS ins, int rep) { 
    char* str = get_copy_of_disasm (ins);
    if (rep) {
	    //we only print the slice once on the first iteration of rep
	    //and also call taint_rep
...
void instrument_taint_add_reg2reg_slice(INS ins, REG dstreg, REG srcreg, int fw_slice)
{
    ...
    UINT32 dst_treg = translate_reg((int)dstreg);
    UINT32 src_treg = translate_reg((int)srcreg);
    UINT32 dst_reg_offset = dst_treg * REG_SIZE;
    UINT32 src_reg_offset = src_treg * REG_SIZE;
    if (REG_is_Upper8(dstreg)) dst_reg_offset += 1;
    if (REG_is_Upper8(srcreg)) src_reg_offset += 1;
    UINT32 size = (dst_regsize < src_regsize) ? dst_regsize : src_regsize;

    INS_InsertCall(ins, IPOINT_BEFORE,
		   AFUNPTR(taint_add_reg2reg_offset),
		   IARG_FAST_ANALYSIS_CALL,
		   IARG_UINT32, dst_reg_offset,
		   IARG_UINT32, src_reg_offset,
		   IARG_UINT32, size,
		   IARG_END);
}
...
void instrument_taint_add_reg2mem_slice(INS ins, REG srcreg, int fw_slice, int set_flags, int clear_flags)
{
    ...
    UINT32 treg = translate_reg((int)srcreg);
    UINT32 reg_off = treg * REG_SIZE;
    if (REG_is_Upper8(srcreg)) reg_off += 1;
...
void instrument_taint_add_mem2reg_slice(INS ins, REG dstreg, int fw_slice)
{
    ...
    UINT32 treg = translate_reg((int)dstreg);
    UINT32 reg_off = treg * REG_SIZE;
    if (REG_is_Upper8(dstreg)) reg_off += 1;
    UINT32 size = (regsize < memsize) ? regsize : memsize;
...
void pred_instrument_taint_reg2reg(INS ins, REG dstreg, REG srcreg, int extend, int fw_slice)
{
    int dst_treg;
    int src_treg;
    UINT32 dst_regsize;
...
TAINTSIGN taint_cmov_memregreg2reg (uint32_t mask, uint32_t dst_reg, u_long mem_loc, uint32_t size, BOOL executed, uint32_t base_reg, uint32_t base_reg_size, uint32_t index_reg, uint32_t index_reg_size) { 
    if (executed) { 
        ...
        if(base_reg_size>0) {
            ...
            taint_add_reg2reg_offset (dst_reg*REG_SIZE, base_reg*REG_SIZE, base_reg_size);
        }
        if (index_reg_size > 0) { 
            assert (index_reg_size == size);
            taint_add_reg2reg_offset (dst_reg*REG_SIZE, index_reg*REG_SIZE, index_reg_size);
        }
    } else { 
        taint_regflag2reg (mask, dst_reg, dst_reg, size);
    }
...
void instrument_pop(INS ins)
{
    ...
    //I think we should clear the source mem for POP, since that memory address is now freed
...
void instrument_imul(INS ins)
{
    ...
    if (count == 2) {
	    ...
	    INS_InsertCall(ins, IPOINT_BEFORE,
			   ...
			   IARG_FAST_ANALYSIS_CALL,
			   IARG_MEMORYREAD_EA,
			   IARG_UINT32, dst_treg*REG_SIZE, // Appears from Mike's code this can not be U8 reg
