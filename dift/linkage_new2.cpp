...
static inline void slice_wait_clock (u_long wait_clock) 
{
    if (wait_clock > checkpoint_clock + 1)  {
        ...
        OUTPUT_SLICE ("[SLICE] #00000000 #add esp, 4 [SLICE_INFO] slice ordering, expected %lu, actual %lu, pid %d, recored_clock %lu\n", current_thread->expected_clock, *ppthread_log_clock, current_thread->record_pid, wait_clock);
...
void PIN_FAST_ANALYSIS_CALL debug_print_inst (ADDRINT ip, char* ins, u_long mem_loc1, u_long mem_loc2, ADDRINT val)
{
	...
	printf ("%s -- img %s static %#x\n", RTN_FindNameByAddress(ip).c_str(), IMG_Name(IMG_FindByAddress(ip)).c_str(), find_static_address(ip));
      ...
      //printf ("mem b66c8f6c tainted %d\n", is_mem_arg_tainted (0xb66c8f6c, 4));
