...
static inline void print_extra_move_reg (ADDRINT ip, int reg, uint32_t reg_size, PIN_REGISTER* regvalue, uint32_t is_upper8, int tainted, string prefix = "[SLICE_EXTRA]") { 
    switch (reg_size) {
        ...
        case 4:
	    if (!tainted) {
		printf ("%s mov $reg(%d,%u), %u //comes with %x\n", prefix.c_str(), reg, reg_size, *regvalue->dword, ip);
	    } else {
		u_long mask = 0, vals = 0;
		int j;
		...
		for (j = 3; j >= 0; j--) {
			...
			vals |= (u_char) regvalue->byte[j];
		    }
		}
		printf ("%s pushfd // comes with %x\n", prefix.c_str(), ip);
		printf ("%s and $reg(%d,%u), 0x%lx // comes with %x\n", prefix.c_str(), reg, reg_size, mask, ip);
		printf ("%s or $reg(%d,%u), 0x%lx // comes with %x\n", prefix.c_str(), reg, reg_size, vals, ip);
...
TAINTINT fw_slice_memmemreg_imm_value (ADDRINT ip, char* ins_str, u_long mem_read, u_long mem_write, uint32_t mem_readsize, uint32_t mem_writesize, 
	...
	if (tainted1 || tainted2 || tainted3) {
		...
		if (!tainted1) print_extra_move_mem (ip, mem_read, mem_readsize);
		if (!tainted2) print_extra_move_mem (ip, mem_write, mem_writesize);
		if (tainted3 != 1) print_extra_move_reg_imm_value (ip, reg, reg_size, regvalue, reg_u8, tainted3);
		return 1;
	}
	return 0;
}
...
TAINTINT fw_slice_memreg_imm_value (ADDRINT ip, char* ins_str, int reg, uint32_t reg_size, uint32_t regvalue, uint32_t reg_u8, u_long mem_loc, uint32_t mem_size) { 
	...
	if (reg_tainted || mem_tainted) {
		...
		if (reg_tainted != 1) print_extra_move_reg_imm_value (ip, reg, reg_size, regvalue, reg_u8, reg_tainted);
		if (mem_tainted == 0) print_extra_move_mem (ip, mem_loc, mem_size);
		return 1;
	}
	return 0;
