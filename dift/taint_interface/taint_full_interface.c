...
{ 
	...
	if (mem_size == 4) {
	    OUTPUT_SLICE_EXTRA (ip, "mov dword ptr [0x%lx], %u", mem_loc, get_mem_value(mem_loc, mem_size));
	} else if (mem_size == 2) {
...
static void init_ctrl_flow_this_branch (ADDRINT ip, const CONTEXT* ctx, std::set<uint32_t> *store_set_reg, std::map<u_long, struct ctrl_flow_origin_value> *store_set_mem)
{
	...
	if (!tainted) { 
	    OUTPUT_SLICE_CTRL_FLOW (0, "mov byte ptr [0x%lx], %u", mem_loc, (UINT8) get_mem_value(mem_loc, 1));
	}
    }
}
