...
{
    if (!tainted) {
	printf ("[SLICE_EXTRA] mov $reg(%d,4), %u //comes with %x\n", reg, value, ip);
    } else {
	...
	printf ("[SLICE_EXTRA] and $reg(%d,4), 0x%lx // comes with %x\n", reg, mask, ip);
	printf ("[SLICE_EXTRA] or $reg(%d,4), 0x%lx // comes with %x\n", reg, (value&valmask), ip);
