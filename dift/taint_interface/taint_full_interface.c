	...
	if (fgets (line, sizeof(line), file) != NULL) {
		...
		if (!strcmp(type, "rangev")) {
		    ...
		    check_map[ip] = tc;
                    ...
                    struct ctrl_flow_param param;
                    param.type = CTRL_FLOW_BLOCK_TYPE_DISTANCE;
...
{
    OUTPUT_SLICE ("[SLICE_EXTRA] push %lu // comes with %x\n", *((u_long *) val), ip);
    OUTPUT_SLICE ("[SLICE_EXTRA] push %lu // comes with %x\n", *((u_long *) (val+4)), ip);
    OUTPUT_SLICE ("[SLICE_EXTRA] push %lu // comes with %x\n", *((u_long *) (val+8)), ip);
    OUTPUT_SLICE ("[SLICE_EXTRA] push %lu // comes with %x\n", *((u_long *) (val+12)), ip);
    OUTPUT_SLICE ("[SLICE_EXTRA] movdqu %s, xmmword ptr [esp] // comes with %x\n", translate_mmx(reg), ip);
    OUTPUT_SLICE ("[SLICE_EXTRA] add esp, 16 // comes with %x\n", ip);
}
...
static inline void add_partial_load_to_slice (uint32_t reg, uint32_t size, char* val, ADDRINT ip)
{
	...
	OUTPUT_SLICE ("[SLICE_EXTRA] push 0x%lx // comes with %x\n", mask, ip);
    }
...
{
    if (is_upper8) {
	OUTPUT_SLICE ("%s mov $reg(%d,-1), %u //comes with %x\n", prefix.c_str(), reg, value, ip);
    } else {
	OUTPUT_SLICE ("%s mov $reg(%d,1), %u //comes with %x\n", prefix.c_str(), reg, value, ip);
    }
}
...
{
    if (!tainted) {
	OUTPUT_SLICE ("%s mov $reg(%d,4), %u //comes with %x\n", prefix.c_str(), reg, value, ip);
    } else {
	...
	OUTPUT_SLICE ("%s pushfd // comes with %x\n", prefix.c_str(), ip);
	OUTPUT_SLICE ("%s and $reg(%d,4), 0x%lx // comes with %x\n", prefix.c_str(), reg, mask, ip);
	OUTPUT_SLICE ("%s or $reg(%d,4), 0x%lx // comes with %x\n", prefix.c_str(), reg, (value&valmask), ip);
	OUTPUT_SLICE ("%s popfd // comes with %x\n", prefix.c_str(), ip);
    }
}
...
{ 
    switch (reg_size) {
        ...
        case 8:
        case 10:
            OUTPUT_SLICE ("%s unhandled size of reg: %d\n", prefix.c_str(), reg_size);
            break;
        case 16:
	    if (!tainted) {
...
static inline void verify_memory (ADDRINT ip, u_long mem_loc, uint32_t mem_size)
{
    OUTPUT_SLICE ("[SLICE_VERIFICATION] pushfd //comes with %x (move upwards)\n", ip); //save flags
    for (uint32_t i = 0; i < mem_size; i++) {
	if (is_mem_tainted(mem_loc+i, 1)) {
	    OUTPUT_SLICE ("[SLICE_VERIFICATION] cmp byte ptr [0x%lx], 0x%x //comes with %x (move upwards)\n", mem_loc+i, *(u_char *) (mem_loc+i), ip);
	    OUTPUT_SLICE ("[SLICE_VERIFICATION] push 0x%lx //comes with %x\n", debug_counter++, ip);
	    OUTPUT_SLICE ("[SLICE_VERIFICATION] jne index_diverge //comes with %x\n", ip);
	    OUTPUT_SLICE ("[SLICE_VERIFICATION] add esp, 4 //comes with %x\n", ip);
	}
    }
    OUTPUT_SLICE ("[SLICE_VERIFICATION] popfd //comes with %x (move upwards)\n", ip); 
    clear_mem_taints (mem_loc, mem_size);
}
...
{
    OUTPUT_SLICE ("[SLICE_VERIFICATION] pushfd //comes with %x (move upwards)\n", ip); //save flags
    OUTPUT_SLICE ("[SLICE_VERIFICATION] cmp $reg(%d,%u),0x%x //comes with %x (move upwards)\n", reg, reg_size, reg_value, ip);
    OUTPUT_SLICE ("[SLICE_VERIFICATION] push 0x%lx //comes with %x\n", debug_counter++, ip);
    OUTPUT_SLICE ("[SLICE_VERIFICATION] jne index_diverge //comes with %x\n", ip);
    OUTPUT_SLICE ("[SLICE_VERIFICATION] add esp, 4 //comes with %x\n", ip);
    OUTPUT_SLICE ("[SLICE_VERIFICATION] popfd //comes with %x (move upwards)\n", ip); 
...
static inline void print_readonly_range_verification (ADDRINT ip, char* ins_str, u_long start, u_long end, u_long mem_loc, uint32_t mem_size, int operand_index = 0)
{
    ...
    OUTPUT_SLICE ("[SLICE_VERIFICATION] pushfd //comes with %x (move upwards), address %lx\n", ip, mem_loc); //save flags
    OUTPUT_SLICE ("[SLICE_VERIFICATION] push eax //comes with %x address %lx, %s\n", ip, mem_loc, ins_str);
    OUTPUT_SLICE ("[SLICE_VERIFICATION] lea eax, %.*s  //comes with %x\n", end_index-start_index+1, start_index, ip);
    OUTPUT_SLICE ("[SLICE_VERIFICATION] cmp eax, 0x%lx //comes with %x\n", start, ip);
    //OUTPUT_SLICE ("[SLICE_VERIFICATION] push eax //comes with %x, input params to index_diverge\n", ip);
    OUTPUT_SLICE ("[SLICE_VERIFICATION] push 0x%lx //comes with %x\n", debug_counter++, ip);
    OUTPUT_SLICE ("[SLICE_VERIFICATION] jb index_diverge //comes with %x\n", ip);
    OUTPUT_SLICE ("[SLICE_VERIFICATION] cmp eax, 0x%lx //comes with %x\n", end, ip);
    OUTPUT_SLICE ("[SLICE_VERIFICATION] jae index_diverge //comes with %x\n", ip);
    OUTPUT_SLICE ("[SLICE_VERIFICATION] add esp, 4 //comes with %x\n", ip);
    OUTPUT_SLICE ("[SLICE_VERIFICATION] pop eax //comes with %x\n", ip);
    OUTPUT_SLICE ("[SLICE_VERIFICATION] popfd //comes with %x (move upwards), address %lx\n", ip, mem_loc); 
}
...
{ 
	...
	if (iter->second.type == CHECK_TYPE_MMAP_REGION) {
		...
		if (!is_mem_tainted(i, 1)) {
		    ...
		    OUTPUT_SLICE ("[SLICE] #00000000 #mov byte ptr [0x%lx], %d [SLICE_INFO] comes with %x\n", i, *(u_char *) i, ip);
...
{
    OUTPUT_SLICE ("[SLICE_ADDRESSING] immediate_address $addr(0x%lx)  //comes with %x (move upwards)\n", mem_loc, ip);
}
...
TAINTSIGN ctrl_flow_init_mem_one_byte (ADDRINT ip, u_long mem_loc, taint_t taint, char origin_value)
{
   if (taint == 0) { 
	...
	OUTPUT_SLICE ("[SLICE_CTRL_FLOW] mov byte ptr [0x%lx], %u //comes with %x\n", mem_loc, (UINT8) origin_value, 0x0000);
   }
}
...
{ 
    if ((current_thread->ctrl_flow_info.is_in_diverged_branch == false && current_thread->ctrl_flow_info.merge_point->empty() == false &&
...
TAINTSIGN print_inst_dest_reg (ADDRINT ip, int reg, PIN_REGISTER* regvalue) 
{
    ...
    if (current_thread->ctrl_flow_info.is_in_diverged_branch == false) {
...
{ 
    if (current_thread->ctrl_flow_info.is_in_diverged_branch == false) {
        ...
        if (current_thread->ctrl_flow_info.diverge_point->empty() == false &&
            ...
            printf ("[CTRL_FLOW] before the divergence, index %lu_%llu, is_rolled_back %d\n", current_thread->ctrl_flow_info.block_index.clock, current_thread->ctrl_flow_info.block_index.index, current_thread->ctrl_flow_info.is_rolled_back);
            if (current_thread->ctrl_flow_info.is_rolled_back == false) { 
                ...
                //let's force to take the alternative path first here
                current_thread->ctrl_flow_info.is_in_diverged_branch = true;
        ...
        ++ current_thread->ctrl_flow_info.block_index.index;
        if (*ppthread_log_clock != current_thread->ctrl_flow_info.block_index.clock) {
            current_thread->ctrl_flow_info.block_index.clock = *ppthread_log_clock;
            current_thread->ctrl_flow_info.block_index.index = 0;
        }
    } else { 
        if (current_thread->ctrl_flow_info.that_branch_block_count == 0) {
            //diverge point for the other branch
            current_thread->ctrl_flow_info.change_jump = true;
            printf ("[CTRL_FLOW] roll back to the diverge point\n");
        }
    }
}
...
{
    if (current_thread->ctrl_flow_info.is_in_diverged_branch == false) {
        //merge point for this branch (the branch that this execution takes)
        if (current_thread->ctrl_flow_info.merge_point->empty() == false &&
            ...
            OUTPUT_SLICE ("[SLICE] #%x #jmp %s_this_branch_end\t [SLICE_INFO] \n", ip, label_prefix);
            OUTPUT_SLICE ("[SLICE] #%x #%s_that_branch_init:\t [SLICE_INFO] \n", ip, label_prefix);
            ...
            OUTPUT_SLICE ("[SLICE] #%x #jmp %s_that_branch_execute_and_taint\t [SLICE_INFO] \n", ip, label_prefix);
            OUTPUT_SLICE ("[SLICE] #%x #%s_that_branch_end:\t [SLICE_INFO] \n", ip, label_prefix);
            OUTPUT_SLICE ("[SLICE] #%x #%s_this_branch_end:\t [SLICE_INFO] \n", ip, label_prefix);
            ...
            current_thread->ctrl_flow_info.that_branch_distance->pop ();
            ...
            printf ("[CTRL_FLOW] This control flow is handled.\n");
            fflush (stdout);
        } 
    } else { 
        //merge point for the other branch
        printf ("[CTRL_FLOW_THE_OTHER_BRANCH] monitor_control_flow_head ip %x, current block count %llu, front %llu\n", ip, current_thread->ctrl_flow_info.that_branch_block_count, current_thread->ctrl_flow_info.that_branch_distance->front());

        if (current_thread->ctrl_flow_info.that_branch_block_count -1 /*minue 1 because we rollback and execute this block twice*/== current_thread->ctrl_flow_info.that_branch_distance->front()) { 
            current_thread->ctrl_flow_info.that_branch_block_count = 0;
            ...
            char label_prefix[32];
            sprintf (label_prefix, "b_%lu_%llu", current_thread->ctrl_flow_info.merge_point->front().clock, current_thread->ctrl_flow_info.merge_point->front().index);
            ...
            taint_ctrl_flow_branch (ip, ctx, 1, current_thread->ctrl_flow_info.that_branch_store_set_reg, current_thread->ctrl_flow_info.that_branch_store_set_mem);
            OUTPUT_SLICE ("[SLICE] #%x #jmp %s_that_branch_end\t [SLICE_INFO] \n", ip, label_prefix);
            OUTPUT_SLICE ("[SLICE] #%x #%s_this_branch_init:\t [SLICE_INFO] \n", ip, label_prefix);
            printf ("[CTRL_FLOW] initialization of original reg/mem values\n");
            ...
            printf ("[CTRL_FLOW] initialization of original reg/mem values done\n");
            OUTPUT_SLICE ("[SLICE] #%x #jmp %s_this_branch_execute_and_taint\t [SLICE_INFO] \n", ip, label_prefix);

            //let's roll back to the diverge point and take the original branch
            current_thread->ctrl_flow_info.change_jump = true;
            current_thread->ctrl_flow_info.is_rolled_back = true;
            ctrl_flow_rollback (&current_thread->ctrl_flow_info.ckpt, current_thread->ctrl_flow_info.that_branch_store_set_mem, 1);
...
TAINTSIGN fw_slice_mem2fpureg (ADDRINT ip, char* ins_str, u_long mem_loc, uint32_t mem_size, uint32_t fp_stack_change, BASE_INDEX_ARGS) 
{ 
...
TAINTSIGN fw_slice_fpureg2mem (ADDRINT ip, char* ins_str, int reg, uint32_t size, const CONTEXT* ctx, uint32_t reg_u8, u_long mem_loc, uint32_t mem_size, uint32_t fp_stack_change, BASE_INDEX_ARGS) 
{
    ...
    fw_slice_reg2mem (ip, slice, reg, size, &regvalue, reg_u8, mem_loc, mem_size, BASE_INDEX_PARAMS);
}
			      ...
			      const PIN_REGISTER* src_regvalue, const PIN_REGISTER* count_regvalue, uint32_t dst_reg_u8, uint32_t src_reg_u8, uint32_t count_reg_u8)
{
	...
	OUTPUT_SLICE ("[SLICE] #%x #%s\t", ip, ins_str);
	OUTPUT_SLICE ("    [SLICE_INFO] #src_regregreg[%d:%d:%u,%d:%d:%u,%d:%d:%u] #dst_reg_value %s, src_reg_value %s, count_reg_value %s\n", 
...
{ 
    ...
    if (flag_tainted) {
	OUTPUT_SLICE ("[SLICE] #%x #%s\t", ip, ins_str);
	OUTPUT_SLICE ("    [SLICE_INFO] #src_regregflag[%d:%d:%u,%d:%d:%u,FM%x:%d:4] #dest_value %s, src_value %s, executed %d\n", 
...
TAINTSIGN fw_slice_flag (ADDRINT ip, char* ins_str, uint32_t mask, BOOL taken, const CONTEXT* ctx) 
{
    if (is_flag_tainted (mask)) {
#ifdef TRACK_CTRL_FLOW_DIVERGE
        if (current_thread->ctrl_flow_info.is_in_diverged_branch == false && current_thread->ctrl_flow_info.change_jump) { 
            ...
            char prefix[64];
            sprintf (prefix,  "b_%lu_%llu", current_thread->ctrl_flow_info.merge_point->front().clock, current_thread->ctrl_flow_info.merge_point->front().index);
            OUTPUT_SLICE ("[SLICE] #0x0 #%s_this_branch_execute_and_taint:\t[SLICE_INFO] \n", prefix);

        } else if (current_thread->ctrl_flow_info.is_in_diverged_branch == true && current_thread->ctrl_flow_info.change_jump) { 
            char changed_inst[64] = {0};
            char* start = strchr (ins_str, ' ');
            if (current_thread->ctrl_flow_info.is_in_diverged_branch_first_inst) {
                ...
                printf ("[CTRL_FLOW] force to jump: eflag value before %x\n", value);
                ...
                printf ("[CTRL_FLOW] force to jump: eflag value after %x\n", value);
            ...
            char prefix[64];
            sprintf (prefix,  "b_%lu_%llu", current_thread->ctrl_flow_info.merge_point->front().clock, current_thread->ctrl_flow_info.merge_point->front().index);
            //well, the process_slice will change this function to take the contrary branch
            printf ("[CTRL_FLOW] now the branch is taken %d\n", (int) taken);
            //well, the process_slice will change this function to take the contrary branch
            OUTPUT_SLICE ("[SLICE] #%x #%s %s_this_branch_init\t", ip, changed_inst, prefix); 
            OUTPUT_SLICE ("    [SLICE_INFO] #src_flag[FM%x:1:4] #branch_taken %d block_index %llu\n", mask, (int) taken, current_thread->ctrl_flow_info.block_index.index);
            OUTPUT_SLICE ("[SLICE] #0x0 #jmp %s_that_branch_init\t[SLICE_INFO] \n", prefix);

            OUTPUT_SLICE ("[SLICE] #0x0 #%s_that_branch_execute_and_taint:\t[SLICE_INFO] \n", prefix);
            current_thread->ctrl_flow_info.change_jump = false;
        } else if (current_thread->ctrl_flow_info.is_in_diverged_branch == true && current_thread->ctrl_flow_info.that_branch_block_count -1 /*minue 1 because we rollback and execute this block twice*/== current_thread->ctrl_flow_info.that_branch_distance->front()) { 
            printf ("[CTRL_FLOW] ignore this jump as it's redundant.\n");
        } else {
#endif
            OUTPUT_SLICE ("[SLICE] #%x #%s\t", ip, ins_str);
            OUTPUT_SLICE ("    [SLICE_INFO] #src_flag[FM%x:1:4] #branch_taken %d \n", mask, (int) taken);
#ifdef TRACK_CTRL_FLOW_DIVERGE
        }
#endif 
    } else { 
...
TAINTSIGN fw_slice_flag2mem (ADDRINT ip, char* ins_str, uint32_t mask, u_long mem_loc, uint32_t mem_size, BASE_INDEX_ARGS) 
{
    ...
    if (is_flag_tainted (mask)) {
	OUTPUT_SLICE ("[SLICE] #%x #%s\t", ip, ins_str);
	OUTPUT_SLICE ("    [SLICE_INFO] #src_flag[FM%x:1:4] #dst_mem[%lx:0:%u]\n", mask, mem_loc, mem_size);
	print_immediate_addr (mem_loc, ip);
	add_modified_mem_for_final_check (mem_loc, mem_size);
    }
}
...
TAINTINT fw_slice_pcmpistri_reg_reg (ADDRINT ip, char* ins_str, uint32_t reg1, uint32_t reg2, uint32_t reg1_size, uint32_t reg2_size, char* reg1_val, char* reg2_val) 
{
	...
	OUTPUT_SLICE ("[SLICE] #%x #%s\t", ip, ins_str);
	OUTPUT_SLICE ("    [SLICE_INFO] #src_regreg_pcmp[i_or_e]stri[%d:%d:%u,%d:%d:%u] #dst_reg_value %.16s, src_reg_value %.16s\n", 
...
TAINTINT fw_slice_pcmpistri_reg_mem (ADDRINT ip, char* ins_str, uint32_t reg1, u_long mem_loc2, uint32_t reg1_size, uint32_t mem_size, char* reg1_val) 
{
	...
	if (reg1_taint != 1) add_imm_load_to_slice (reg1, 16, reg1_val, ip);
        if (mem_taints == 2) fprintf (stderr, "[BUG] pcmpistri has partially tainted memory?\n");
        if (!mem_taints) { 
...
{ 
	...
	if (mem_tainted) {
	    OUTPUT_SLICE ("[SLICE] #%x #%s\t", ip, ins_str);
	    OUTPUT_SLICE ("    [SLICE_INFO] #src_mem[%x:%d:%lu] #ndx_reg[%d:%d:%d,%d:%d:%d,%d:%d:%d]\n", mem_loc, mem_tainted, stringlen, LEVEL_BASE::REG_ECX, ecx_tainted, 4,
...
{ 
	...
	if (mem_tainted) {
	    OUTPUT_SLICE ("[SLICE] #%x #%s\t", ip, ins_str);
	    OUTPUT_SLICE ("    [SLICE_INFO] #src_mem[%x:%d:%u] #ndx_reg[%d:%d:%d,%d:%d:%d,%d:%d:%d] (origin_mem_src %x)\n", src_mem_loc, mem_tainted, size, LEVEL_BASE::REG_ECX, ecx_tainted, 4,
