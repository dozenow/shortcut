...
def compSlice (outputdir, strno, pid):
    return os.system("gcc -masm=intel -c -fpic -Wall -Werror "+outputdir+"/exslice" + strno + "." + pid + ".c.d.c -o "+outputdir+"/exslice" + strno + "." + pid + ".o")
import glob
import string
...
if args.compile_all:
    input_asm_file = glob.glob(outputdir + '/exslice[123456789]*.*.c.d.c')
...
for asm_file in input_asm_file:
    ...
    record_pid = asm_file[asm_file.rfind (".", 0, -7)+1:-6]
