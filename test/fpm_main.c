			...
			//xdou timer
			{
...

				{
					...
					if (hndl == NULL && gid[0] != '0') { 
						...
						struct mmap_region_info infos[32]; 
						int region_count = 0;
						int i = 0; 
						u_long addr, size, rc = 0; 
#include <sys/mman.h> 
						proc_file = fopen ("/proc/sys/kernel/jumpstart_load_slice_pid", "r");
						ret_str = fgets (pid, sizeof(pid), proc_file);
						if (ret_str == NULL) fprintf (stderr, "cannot read from jumpstart_load_slice_pid\n");
						pid[strlen(pid)-1] = '\0';
						fclose (proc_file);
						//before we load the slice, we mmap extra memory regions that will be used in the code region
						//unload the extra memory regions in the **kernel**  (to avoid slice stack occupying these regions)
						sprintf (filename, "/replay_logdb/rec_%s/extra_mmap_region", gid);
						extra_fd = open (filename, O_RDONLY);
						rc = read (extra_fd, infos, sizeof(infos));
						if (rc == sizeof(infos)) fprintf (stderr, "ok...the extra_mmap_region file is too big.\n");
						...
						//now let's load the slice
						sprintf (filename, "/replay_logdb/rec_%s/exslice.%s.so", gid, pid);
						//fprintf (stderr, "loading slice %s\n", filename);
						hndl = dlopen (filename, RTLD_NOW);
						if (hndl == NULL) {
						...
						for (i = 0; i < region_count; ++i) { 
							...
							if (ret != 0) fprintf (stderr, "munmap reservation fails for extra mmap region, addr %lx ret %d\n", infos[i].addr, ret);
						}
					}
