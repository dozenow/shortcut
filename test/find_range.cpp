...
int handle_loop (FILE* file, u_long target_inst, unsigned long long start_bb, unsigned long long& extra_bb, int skew, const char* rwname, int& max_write) 
{
    int iter_count = 1;
...
int main (int argc, char* argv[])
{
	...
	if (!strncmp(line1, "[MERGE]", 7)) {
		...
		if (bb1 != bb2+skew || clock1 != clock2) {
		    fprintf (stderr, "Unecpected difference in clock at divergence\n%s%s", line1, line2);
		    return -1;
		}
		if (taken1 != taken2) {
			...
			handle_loop (file2, inst1, bb2, extra_bb, skew, argv[2], max_write);
			skew -= extra_bb;
			DPRINT ("Skew is now %ld\n", skew);
		    } else if (loop2 && null2) {
			unsigned long long extra_bb;
			handle_loop (file1, inst2, bb1, extra_bb, 0, argv[1], max_write);
			skew += extra_bb;
			DPRINT ("Skew is now %ld\n", skew);
		    } else {
		...
		if (ea1 != ea2) {
		    DPRINT ("Read at address %lx bb %lld clock %ld: %lx vs %lx\n", inst1, bb1, clock1, ea1, ea2);
		    if (ea1 > ea2) {
